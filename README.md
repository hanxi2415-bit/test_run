...
- 训练集、验证集、测试集应按时间顺序划分。除非明确说明泄漏风险，否则不要在时间序列任务中使用随机验证集。
- 每个模型都应和简单基线比较，例如“预测收益率恒为 0”，而不只是互相比较 ML 模型。

## 当前局限

- Notebook 仍依赖本地已准备好的 Qlib provider 目录。
- 单只股票样本较小，不适合得出很强的模型优劣结论。
- 当前指标应视为探索结果；在干净环境中完整复跑前，不建议作为正式实验结论。
